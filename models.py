...
class Net(nn.Module):
    def __init__(self):
        ...
        # the output Tensor for one image, will have the dimensions: (1024, 8, 8)
        self.drop6 = nn.Dropout(p=0.1)

        self.conv7 = nn.Conv2d(1024, 2048, 1)
        # output size = (W-F)/S +1 = (8-1)/1 + 1 = 8
        # the output Tensor for one image, will have the dimensions: (2048, 8, 8)
